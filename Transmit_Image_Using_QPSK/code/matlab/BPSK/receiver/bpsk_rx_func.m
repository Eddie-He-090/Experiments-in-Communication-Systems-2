...
local_sync=local_sync+1i*local_sync;
rx_signal=rxdata;
%% matched filtering
...
%% package search
[rx_frame,cor_abs,th_max,index_s]=rx_package_search(rx_sig_down,local_sync,20687); 
if max(cor_abs)>100
    ...
    %% time domain equalize 时域均衡
    out_signal8=rx_time_equalize(out_signal7);
    %% signal demod 解调
    [soft_bits_out,evm] = rx_qpsk(out_signal7);
    Si=[1 1 0 1 1 0 0];
    m=0;  %% 解扰
